...
namespace BDCommon
{
    public abstract class TSCodecHEVC
    {
        ...
        public static void Scan(TSVideoStream stream, TSStreamBuffer buffer, BDInfoSettings settings, ref string tag)
        {
            ...
            if (settings.ExtendedStreamDiagnostics && !stream.IsInitialized)
            {
        ...
        private static string SliceSegmentLayer(TSStreamBuffer buffer, long nalUnitType)
        {
            ...
            var _firstSliceSegmentInPicFlag = buffer.ReadBool();

            if (nalUnitType >= 16 && nalUnitType <= 23)
                tempBool = buffer.ReadBool(); // no_output_of_prior_pics_flag

            var _slicePicParameterSetId = buffer.ReadExp(true);
